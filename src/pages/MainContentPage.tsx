...
import food from "../assets/chickenplate.jpg"
import Item from "../components/Item";
import data from "../utils/data.json";
import { motion } from "framer-motion"
import { useState } from "react";
export default function MainContentPage({type}:{type:string}) {

 const [dataOnView , setDataOnView] = useState(0)
 
 function nextDataOnView(){
    setDataOnView(prev => prev + 1 >= 11 ? 0 : prev+1)
 }
 function prevDataOnView(){
    setDataOnView(prev => prev - 1 <= 0 ? 11 : prev - 1 )
 }

    return <div >
        <div className="text-center bg h-screen bg-cover bg-center bg-blend-multiply bg-linear-to-t from-black/90 to-transparent" style={{ backgroundImage: `url(${food})` }}>
            <motion.div
            ...
            >
                <h2 className="sm:pt-[20%] pt-[50%] text-primary">FINE DINING EXPERIENCE</h2>
                <h1 className="text-5xl font-bold">{data[dataOnView].description}</h1>
                <p className="flex items-center gap-2 mx-auto place-content-center">
                    ...
                    <span className="text-primary">PREPARATION TIME : {data[dataOnView].prep_time}</span>
                </p>
        ...
        <div className="grid sm:grid-cols-2 mx-4 gap-2">
            {
                data[dataOnView].items.map((item: any) => <Item name={item.name} price={item.price} frenchTr={item.name_fr} description={item.description} />)
            }
        </div>
